...
model_file_path = os.path.join(script_dir, "highestTestModelHeartblood.joblib")
scaler_file_path = os.path.join(script_dir, "HeartbloodScaler.joblib")
# Load the models
model = joblib.load(model_file_path)
scalerpath = joblib.load(scaler_file_path)
...
def predict_heartblood():
    ...
    scaledInput = scalerpath.transform([list(user_input)])
    user_input=scaledInput
    

    # Make prediction using the model
    prediction = model.predict(user_input)
    decision_function_scores = model.decision_function(user_input)
